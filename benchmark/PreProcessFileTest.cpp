...
void PreProcessFileByScanDir(benchmark::State &state) {
  for (auto _ : state) {
    uint32_t file_count = scandir(g_file_path_in.c_str(), &g_file_name_list,
                                  FileNameFilter, alphasort);
    if (file_count < 0) {
      std::cout << __FILE_NAME__ << ":" << __LINE__
                << ":Read file name is error!" << std::endl;
    }
  ...
  for (auto _ : state) {
    ...
    while (fgets(file_name, 256, pipe_ptr)) {
      ...
      if (full_file_path.find(".yuv") != std::string::npos) {
        ...
        g_file_path_vec.emplace_back(
            std::move(full_file_path.substr(0, full_file_path.size() - 1)));
      }
    }
    pclose(pipe_ptr);
