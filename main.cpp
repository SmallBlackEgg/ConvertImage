...
int main(int argc, char *argv[]) {
  ...
  {
    std::cout << __FILE_NAME__ << ":" << __LINE__ << ":The command line is error!" << std::endl;
    return -1;
  }
  arg = arg.substr(pos + 2);
  std::size_t equal_pos = arg.find_first_of("=");
  if(equal_pos == std::string::npos || equal_pos == 0 || (equal_pos >= arg.length() -1))
  {
    std::cout << __FILE_NAME__ << ":" << __LINE__ << ":The command line is error!" << std::endl;
    return -1;
  }
  ...
  {
    file_read_ptr = new LiveYUV(RunTimeConfig::GetInstance().GetConvertConfig().file_path_in);
  }
  file_read_ptr->Run();
